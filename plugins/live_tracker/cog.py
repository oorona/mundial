...
def determined_third_slots(finished_thirds):
...
class LiveTracker(commands.Cog):
    ...
    async def _resolve_bracket(self, s):
        ...
        # across every still-possible qualifying combination, so a winner-vs-third match
        # can fill in before the last group is final; a "3rd Group …" slot's occupant is
        # keyed off the OTHER side's "Winner Group X" label.
        third_by_winner = determined_third_slots(third)
